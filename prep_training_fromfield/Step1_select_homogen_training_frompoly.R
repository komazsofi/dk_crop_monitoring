...
validity=as.data.frame(st_is_valid(poly))

invalid_poly=poly[validity$`st_is_valid(poly)`=="FALSE",]
st_write(invalid_poly,"invalid_poly.shp")

valid_poly=poly[validity$`st_is_valid(poly)`=="TRUE",]
valid_poly_inv_buff=st_buffer(valid_poly,-30)

st_write(valid_poly_inv_buff,"valid_poly_inv_buff.shp")


poly_inv_buff=st_buffer(poly,-30)

x_as_matrix <- st_coordinates(poly)[,1:2]
min_box <- getMinBBox(x_as_matrix)

a=min_box_sf(poly[1,])

ggplot() + 
  geom_sf(data = poly[1000,], 
          fill = 'red', 
          alpha = .2) + 
  geom_sf(data = min_box_sf(poly[1000,]), 
          fill = NA) 